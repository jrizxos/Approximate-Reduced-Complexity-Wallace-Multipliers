''' Generates the cpp code for /cppsim/mulmul_size.cpp and mul8.cpp
...
mul_size = 16 # set this to 8, 16 or 32

vhd_f = open('axc_wallace_'+str(mul_size)+'x'+str(mul_size)+'.vhd') # select the appropriate file
    ...
    if(line.startswith('--')):
        ...
        if('\'0\'' in l_lst[2]):
            ...
            if(l_lst[2].startswith('p')):
                ...
                numA = int(num/mul_size)
                numB = num%mul_size
                shift = '' if(idx==0) else ' << '+str(idx)
                d_type = 'int' if(idx<31) else 'uint64_t'
                c_line += '('+d_type+')partials[' + str(numA) + '][' + str(numB) + '])' + shift + ';'
            else:
                idx = int(l_lst[0][l_lst[0].index('(')+1:l_lst[0].index(')')])
                if((idx)>30):
                        c_line += '(uint64_t)'
...
print('ADDER_MAP_'+str(mul_size)+' =',ADDER_MAP)

print('ADDER_MAP_'+str(mul_size)+' len =',len(ADDER_MAP))